...
const values = [
  { icon: '♧', title: 'Comunidad', text: 'Fomentamos conexiones reales entre estudiantes para apoyarnos y crecer juntos.' },
  { icon: '♢', title: 'Confianza', text: 'Promovemos un entorno seguro, transparente y respetuoso en cada interacción.' },
  { icon: '♧', title: 'Oportunidad', text: 'Impulsamos el talento y facilitamos el acceso a soluciones que generan impacto.' },
];
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-3 ">
      <div className="flex items-center gap-4">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#f0eaff] text-2xl text-[#7b32ca]">{icon}</span>
    ...
    <section className="border-b border-slate-100 bg-linear-to-r from-white via-[#f8f7ff] to-[#e9e4ff]">
      <div className="mx-auto flex min-h-[250px] w-[calc(100%-48px)] max-w-6xl items-center justify-between gap-8 max-lg:min-h-[280px] max-md:flex-col max-md:items-start max-md:py-7">
        ...
        <div className="relative flex h-[180px] w-[390px] max-w-full items-center justify-center overflow-hidden max-md:hidden">
          ...
          <span className="absolute left-5 top-7 rounded-full bg-white p-3 text-xl text-[#7b32ca] shadow">⚒</span><span className="absolute right-5 top-8 rounded-full bg-white p-3 text-xl text-[#7b32ca] shadow">▱</span><span className="absolute bottom-5 left-12 rounded-full bg-white p-3 text-xl text-[#7b32ca] shadow">✎</span><span className="absolute bottom-4 right-12 rounded-full bg-white p-3 text-xl text-[#7b32ca] shadow">▣</span>
        </div>
      </div>
    </section>
  );
