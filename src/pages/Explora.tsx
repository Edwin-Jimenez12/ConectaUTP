...
export function Explora() {
  ...
  const filteredServices = useMemo(() => services
    ...
    .sort((a, b) => Number(Boolean(b.is_featured)) - Number(Boolean(a.is_featured)) || Number(Boolean(b.priority_results_enabled)) - Number(Boolean(a.priority_results_enabled)) || Number(Boolean(b.profile_boost_enabled)) - Number(Boolean(a.profile_boost_enabled)) || (b.featured_priority ?? 0) - (a.featured_priority ?? 0) || b.created_at.localeCompare(a.created_at)), [category, query, services]);
