...
interface PublicProfile {
  ...
  career: string | null;
  faculty: string | null;
  institutional_email_status: string;
}
...
export function PerfilPublico({ profileId }: { profileId: string }) {
  ...
  if (!profile) return <div className="mx-auto min-h-[620px] max-w-[760px] px-6 py-12 text-sm text-[#676878]">Perfil no disponible o privado.</div>;
  ...
  return (
    <section className="mx-auto min-h-[620px] w-[calc(100%-48px)] max-w-[760px] py-10">
      ...
      <header className="mt-5 rounded-xl border border-slate-200 bg-white p-6"><div className="flex items-center gap-4"><span className="h-16 w-16 rounded-full bg-[#eeeaff]" /><div><h1 className="text-2xl font-semibold">{displayName}</h1><p className="text-sm text-[#676878]">@{profile.username ?? 'usuario'}</p></div></div>{profile.institutional_email_status === 'verified' && <span className="mt-4 inline-block rounded bg-[#eaf8ee] px-2 py-1 text-xs text-[#268044]">Comunidad UTP verificada</span>}<p className="mt-4 text-sm text-[#676878]">{profile.bio || 'Este usuario todavía no ha agregado una biografía.'}</p>{profile.career && <p className="mt-3 text-xs text-[#676878]">{profile.career}{profile.faculty ? ` · ${profile.faculty}` : ''}</p>}</header>
