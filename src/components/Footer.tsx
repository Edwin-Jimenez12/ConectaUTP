      ...
      <div className="relative mx-auto w-[calc(100%-48px)] max-w-7xl py-5">
        <div className="grid grid-cols-[1.4fr_0.8fr_1fr] gap-12 max-lg:grid-cols-2 max-md:grid-cols-1 max-md:gap-9">
          <div className="max-w-sm">
            ...
            {!session && <a className="mt-6 inline-flex min-h-10 items-center rounded-md border border-white/30 px-4 text-sm font-semibold text-white transition-colors hover:border-white hover:bg-white hover:text-[#182044] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white" href="#registro">
...
function SocialLink({ children, href, label, external = false }: { children: ReactNode; href: string; label: string; external?: boolean }) {
  return <a className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white/80 transition-all hover:-translate-y-1 hover:border-white hover:bg-white hover:text-[#182044] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white" href={href} aria-label={label} target={external ? '_blank' : undefined} rel={external ? 'noreferrer' : undefined}>{children}</a>;
}
